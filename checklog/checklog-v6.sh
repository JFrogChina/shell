#!/bin/bash
LOG_FILE="request-all.log"  # 替换为实际的日志文件路径
...
current_date=${timestamp:0:8}
current_hour=${timestamp:9:2}
...
while ((current_line <= MAX_LINES)) ; do
  ...
  # 20230524204238|25|REQUEST|10.190.15.220|anonymous|GET|/public/commons-digester/commons-digester/1.8/commons-digester-1.8.pom.sha1|HTTP/1.1|200|40
  # 20230524204242|1|REQUEST|172.24.149.91|niuruili|PUT|/libs-snapshot/com/didi/mlcp/drtc_pt/1.0.0/drtc_pt-1.0.0.jar|HTTP/1.1|409|6222

  IFS="|" read -r timestamp req_duration request  remote_addr  username  method url user_agent status   req_content_length;

res_content_length=$req_content_length

#echo ${req_content_length}

  # 提取日期和小时
  log_date=${timestamp:0:8}
  log_hour=${timestamp:9:2}
  ...
  if [[ "$is_different" == "true" ]]; then
    ...
    echo "$current_date, $current_date"_"$current_hour_plus_hours, $request_length_mb, $response_length_mb" >> checklog-result.csv
      echo "$request_length_sum ${response_length_sum}"
    # 更新当前日期和小时，并重置统计变量
